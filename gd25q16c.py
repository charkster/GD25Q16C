...
#		if (self.check_wip_and_wel(expected_status=0x02, timestep = 0.00001, debug=debug) == 0):
	...
	def read_status_register_lower(self, debug=False):
		...
		list_of_bytes = self.spi.xfer2([self.CMD_Read_Status_Register_Lower, 0x00])
	...
	def read_status_register_upper(self, debug=False):
		...
		list_of_bytes = self.spi.xfer2([self.CMD_Read_Status_Register_Upper, 0x00])
		return list_of_bytes[1]
	...
	def write_status_register(self, write_bytes = [0x00, 0x00], debug=False):
		if (debug or self.debug):
			...
			print(write_bytes)
		self.write_enable()
		self.spi.xfer2([self.CMD_Write_Status_Register] + write_bytes)
